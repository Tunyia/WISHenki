...
Интерактивное консольное меню для управления БД: студенты, аккаунты, запись на мероприятия, посещения, удаление, сброс пароля.

### Запуск (локально, Docker)

Из **корня** репозитория, контейнеры должны быть запущены (`docker compose up -d`):

```powershell
cd C:\Users\tyomk\Desktop\WISHenki
docker compose build api
docker compose up -d --force-recreate api
docker compose exec -it api python admin_menu.py
```

Флаг **`-it`** обязателен — без него меню не интерактивное (Docker выдаст предупреждение и завершится).

Выход из меню: пункт **0** в главном меню или `Ctrl+C`.
...
После обновления кода на сервере, если скрипт новый:
...
| Раздел | Действия |
|--------|----------|
| **Студенты** | все / с аккаунтом / без аккаунта, поиск, карточка студента |
| | удалить только аккаунт (студент остаётся в рейтинге) |
| | сбросить пароль, удалить студента из БД полностью |
| **Мероприятия** | список предстоящих и прошедших, участники |
| | записать / выписать (предстоящие) |
| | отметить / снять посещение + бонус (архив) |
| **Аккаунты** | список, удаление по user_id или email |
| **Статистика** | количество студентов, аккаунтов, мероприятий, списаний |
| **Списания / мерч** | все списания (мерч + магазин), откат заказа с возвратом вишенок |

### Частые задачи

**Отменить заказ мерча и вернуть вишенки** — главное меню → **5** → **7** (ID заказа, подтверждение «д»).

**Посмотреть все списания** — **5** → **1** (мерч и транзакции магазина в одном списке).

**Пользователь забыл пароль** — меню → **1** (Студенты) → **7** (сбросить пароль)  
или **3** (Аккаунты) → удалить аккаунт → пользователь регистрируется заново.

**Записать студента на мероприятие** — **2** → **5** (ID мероприятия и студента).

**Удалить человека из рейтинга полностью** — **1** → **8** (необратимо).

> Для быстрого просмотра списков без меню используйте `list_users.py` (см. выше).
